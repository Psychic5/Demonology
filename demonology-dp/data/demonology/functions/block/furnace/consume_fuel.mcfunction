data merge storage demonology:storage {list:[]}
data modify storage demonology:storage list set from block ~ ~ ~ Items
...
data modify block ~ ~ ~ Items set from storage demonology:storage list

execute store result score @s demonology.temp run data get block ~ ~ ~ Items[{Slot:1b}].Count
scoreboard players remove @s demonology.temp 1
execute store result block ~ ~ ~ Items[{Slot:1b}].Count byte 1 run scoreboard players get @s demonology.temp
